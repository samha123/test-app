                        ...
                        <tbody>
                            @foreach($orders as $order)
                                <tr>
                                  ...
                                  <td>{{ $order->et_price + $order->ce_price + $order->product_price  }}</td>
                                
                                    
                                    <td>{{ OrderConstants::ORDER_STATUS[$order->status] }}</td>
                                    <td>{{ $order->payment_status}}</td>
                                  <td>@if(OrderConstants::ORDER_STATUS[$order->status]==="Delivered")<a href="{{route('admin.salesorder.invoice.download',['sales_order_id'=> $order->id])}}"><i class="fa fa-file">Download Invoice</i></a>@endif</td>
                                    <td class="text-right">
                                        <a href="{{ route('admin.order.show', $order->id) }}" class="btn btn-sm btn-outline-warning "><i class="fa fa-eye"></i></a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
